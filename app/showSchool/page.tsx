...
export default function ShowSchool () {
  ...
  return (
    <main className="lg:px-16 md:px-8 px-6 pt-14">
      ...
      <button className="bg-green-600 text-white py-2 px-4 rounded-md cursor-pointer hover:bg-green-700">+ Add New School</button>
      </Link>
      ...
      {schools.length === 0 ? (
        <p>No schools available.</p>
          ...
          {schools.map((school) => (
            <li key={school.id} className="max-w-full flexflex-col bg-white p-6 pointer-events-auto transform scale-100 opacity-100 transition-all duration-300 ease-in-out drop-shadow-[0_0_5px_rgba(0,0,0,0.3)] hover:shadow-lg rounded-lg relativ">
  <button onClick={() => handleRemove(school.id)}
  ...
  ✕
    ...
    {school.city}, {school.state}
              ...
              <h3 className="text-2xl font-bold text-black">{school.name}</h3>
                    <div className="flex justify-between w-full mt-4 bg-fucsia-50 p-">
<p  className="font-semibold text-black">Address</p>
  <p className="text-black">{school.address}</p>
</div>
              <div className="flex justify-between w-full mt-2 bg-fuchsa-50 p-">
<p  className="font-semibold text-black">Mobile No.</p>
  <p className="font-medium text-black">{school.contact}</p>
              </div>
              <div className="flex justify-between w-full mt-2 bg-fuchsa-50 p-">
                <p  className="font-semibold text-black">Email ID</p>
  <p className="font-medium text-black">{school.email_id}</p>
              </div>
